...
def before_build():
    ...
    try:
        ...
        frappe.log_error(f"Frontend build failed: {str(e)}", "Careverse HQ Build Error")
...
def after_build():
    ...
    try:
        ...
        frappe.log_error(f"Build verification failed: {str(e)}", "Careverse HQ Build Verification")
...
def call_frontend_build_script():
    ...
    # which is the careverse_hq subdirectory, so we need to go up one level
    try:
        app_path = frappe.get_app_path("careverse_hq")
...
def build_frontend_assets():
    ...
    print("Building Careverse HQ frontend assets...")
...
def copy_html_to_www():
    ...
    source_html = os.path.join(app_path, "careverse_hq", "public", "admin-central", "index.html")

    # Destination: www directory
    dest_html = os.path.join(app_path, "careverse_hq", "www", "admin-central.html")
...
def verify_build_assets():
    ...
    assets_dir = os.path.join(app_path, "careverse_hq", "public", "admin-central", "assets")
    if not os.path.exists(assets_dir):
        raise Exception("Assets directory not found after build")

    # Check if HTML file exists
    html_file = os.path.join(app_path, "careverse_hq", "www", "admin-central.html")
...
def run_frontend_build():
    ...
    """
    try:
        frappe.logger().info("Starting Careverse HQ frontend build after migration...")

        # Call the existing frontend build script function
        # This function handles all the build logic and error reporting
        call_frontend_build_script()

        frappe.logger().info("Careverse HQ frontend build completed successfully after migration")

    except Exception as e:
        # Log the error but don't fail the migration
        frappe.log_error(
            f"Frontend build failed after migration: {str(e)}",
            "Careverse HQ Post-Migration Build Error"
        )
