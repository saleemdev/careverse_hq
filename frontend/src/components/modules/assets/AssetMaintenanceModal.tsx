...
const AssetMaintenanceModal: React.FC<Props> = ({ open, onClose, assetName, company }) => {
    ...
    const [tasks, setTasks] = useState<any[]>([{ key: Date.now() }]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const taskData = tasks.map((_, idx) => ({
                maintenance_task: values[`task_name_${idx}`],
                maintenance_type: values[`task_type_${idx}`],
                start_date: values[`task_start_${idx}`]?.format('YYYY-MM-DD'),
                end_date: values[`task_end_${idx}`]?.format('YYYY-MM-DD'),
                periodicity: values[`task_periodicity_${idx}`],
                assign_to: values[`task_assign_${idx}`],
                description: values[`task_desc_${idx}`] || '',
            }));
                ...
                {tasks.map((task, idx) => (
                    <Card
                        ...
                        extra={tasks.length > 1 && (
                            <Button type="text" danger icon={<DeleteOutlined />} onClick={() => removeTask(idx)} />
                        )}
                    >
                        <Space direction="vertical" style={{ width: '100%' }} size={8}>
                            <Form.Item name={`task_name_${idx}`} label="Task Name" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                                <Input placeholder="e.g. Monthly Inspection" />
                            </Form.Item>
                            <Space wrap style={{ width: '100%' }}>
                                <Form.Item name={`task_type_${idx}`} label="Type" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                                    <Select style={{ width: 200 }} options={MAINTENANCE_TYPES.map((t) => ({ value: t, label: t }))} />
                                </Form.Item>
                                <Form.Item name={`task_periodicity_${idx}`} label="Periodicity" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                                    <Select style={{ width: 150 }} options={PERIODICITY_OPTIONS.map((p) => ({ value: p, label: p }))} />
                                </Form.Item>
                            </Space>
                            <Space wrap style={{ width: '100%' }}>
                                <Form.Item name={`task_start_${idx}`} label="Start Date" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                                    <DatePicker />
                                </Form.Item>
                                <Form.Item name={`task_end_${idx}`} label="End Date" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                                    <DatePicker />
                                </Form.Item>
                                <Form.Item name={`task_assign_${idx}`} label="Assign To" rules={[{ required: true }]} style={{ marginBottom: 8 }}>
                            ...
                            <Form.Item name={`task_desc_${idx}`} label="Description" style={{ marginBottom: 0 }}>
